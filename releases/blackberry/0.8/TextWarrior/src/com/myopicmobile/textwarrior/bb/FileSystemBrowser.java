...
public class FileSystemBrowser extends MainScreen implements TextWarriorStringsResource{ 
    ...
    private final static int RENAME_MODE = 1;
    ...
    protected void createFilenameInputField(){
        _filenameInputField = new BasicEditField(_appStrings.getString(FILE_SYSTEM_BROWSER_LABEL_FILENAME),
    ...
    private void setRenameMode(){
        ...
        if (_selectMode != RENAME_MODE){
            _selectMode = RENAME_MODE;
    	...
    	if (choice.equals(GO_UP_DIR)){
    		...
    		try {
    	        ...
    	        if (fileConnection.isDirectory()) {
    	        	...
    	        	else if (_selectMode == RENAME_MODE){
    	        		if (choice.equals(_filenameInputField.getText())){
        	        		// user selected the same file choice again
    	                	onFilenameConfirm();
    	        		}
    	        		else{
    ...
    //TODO flaky way of determining if an entry refers to a directory
