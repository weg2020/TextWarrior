...
implements TextWarriorStringsResource, FieldChangeListener{
	...
	protected void sublayout(int width, int height){
		...
		x = FIELD_PADDING;
		int row0Height = Math.max(_findLabel.getHeight(), _findPrevious.getHeight());
		y += row0Height + FIELD_PADDING + 2;
		// right align label
		...
		int row1Height = Math.max(_replaceLabel.getHeight(), _replace.getHeight());
		y += row1Height + FIELD_PADDING + 2;
	...
	public void fieldChanged(Field field, int context){
		if ((field == _findNext || field == _searchedText) &&
			...
			_controller.replace(getReplacementText(), true);
		}
