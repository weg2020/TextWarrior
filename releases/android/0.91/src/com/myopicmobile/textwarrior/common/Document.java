...
public class Document extends TextBuffer {
	...
	public Document(TextFieldMetrics metrics, boolean isWordWrap){
		super();
		_metrics = metrics;
		_isWordWrap = isWordWrap;
		
		if(_isWordWrap){
			//create row table for an empty buffer ie with only 1 entry
			_rowTable = new ArrayList<Integer>();
			_rowTable.add(0);
		}
	}
	...
	public void analyzeWordWrap(){
		...
		if(maxWidth < 2*_metrics.getAdvance('M') ){
			TextWarriorException.assertVerbose(false,
				"Word wrap failed because there is not enough space for the widest char");
			return;
		}
